...
abstract class BaseAppWidget : AppWidgetProvider() {
    ...
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager,
        ...
        val updateIntent = Intent(Contacts.APP_WIDGET_UPDATE)
        updateIntent.putExtra(Contacts.EXTRA_APP_WIDGET_NAME, NAME)
    ...
    fun notifyChange(service: MusicService, what: String) {
        if (hasInstances(service)) {
            if (Contacts.META_CHANGED == what || Contacts.PLAY_STATE_CHANGED == what) {
    ...
    protected fun getAlbumArtDrawable(resources: Resources, bitmap: Bitmap?): Drawable {
        return if (bitmap == null) {
            ContextCompat.getDrawable(MyApplication.context, R.drawable.default_album_art)!!
        } else {
